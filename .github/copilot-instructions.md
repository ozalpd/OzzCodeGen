...
- **Pluggable engines:** Engine IDs are centralized in [EngineTypes.cs](OzzCodeGen/CodeEngines/EngineTypes.cs); the WPF UI binds to these IDs and injects engine-specific UIs.
  ...
  - `WpfMvvmCodeEngine` generates WPF Create/Edit views, ViewModels, command classes, and per-entity lookup service interfaces/implementations (controlled by `GenerateLookupService`). It also generates shared MVVM infrastructure in `InfrastructureFolder`.
  ...
  - Prefer engine-specific entity/property settings instead of generic `EntitySetting` and `PropertySetting` types.
...
- **Repository base layer:** Shared repository code generation abstractions live in `OzzCodeGen/CodeEngines/CsDbRepository/`. SQLite repository generation builds on these base classes. Repository contracts (interfaces) can be generated into a separate folder and namespace from the implementations; keep folder/namespace resolution centralized and consistent across engine settings, templates, and UI.
- **MVVM engine base layer:** Shared MVVM engine behaviors live in `OzzCodeGen/CodeEngines/Mvvm/` (`BaseMvvmCodeEngine`, `BaseMvvmEntitySetting<T>`, `BaseMvvmPropertySetting`, `IMvvmEntitySetting`). These are platform-agnostic so a future MAUI engine can inherit from the same base classes. The WPF-specific engine (`WpfMvvmCodeEngine`) lives under `OzzCodeGen/CodeEngines/WpfMvvm/` and extends these bases with WPF-specific settings, templates, and UI.
- **App infrastructure base layer:** Shared infrastructure-folder behavior for app-language engines (for example C#/TypeScript style generators) should live in `BaseAppInfraCodeEngine`. Use this for engines that need `InfrastructureFolder`/`TargetInfrastructureDirectory` behavior; avoid it for engines that do not generate app-layer infrastructure (for example `CSharpModelClassCodeEngine`, `ResxEngine`, and SQL script engines).
...
- Current version alignment:
  - `OzzCodeGen` and `OzzCodeGen.Wpf`: `2.3.1`
...
- `OzzCodeGen.Wpf` UI icons use Bootstrap path resources declared in `Resources/BootstrapIcons.xaml`. Icon pack: **Bootstrap Icons v1.13.1** (source: https://icons.getbootstrap.com, license: MIT).
