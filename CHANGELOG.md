...
- Added `HasErrors` property to `IIsDirty` interface in `WpfBaseVmTemplate.tt`.

## [2.3.10] - 2026-05-26

### Added
- Introduced Detail View generation mode and column layout options
...
- Introduced `SkipManuallyEdited` to `FileGenerationMode` and updated  `FileGenerationMode` to WPF MVVM template constructors and logic, enabling fine-grained control over file generation and overwrite behavior. Updated template instantiations and rendering methods to respect this mode.
...
- Introduced `GenModeLoadCommand` and `PageCommand` enum, updated UI for per-entity configuration, and extended templates and code generation logic to support paging commands with correct inheritance and parameter handling.
...
- Introduced `CSharpViewTemplate` and `XamlViewTemplate` for generating WPF MVVM entity views (code-behind and XAML).
...
- Commented out future infrastructure settings in `WpfMvvmEngineUI`.
